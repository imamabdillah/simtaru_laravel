...
<script>
    ...
    $(document).ready(function() {
        ...
        map.on('click', function(e) {
            $('input[name="krk_lat"]').val(e.latlng.lat);
            $('input[name="krk_lng"]').val(e.latlng.lng);
    ...
    function get_map(cb, geo_url, geo_prefix, geo_id) {
        ...
        function osm_info(e) {
            ...
            if (cb.hasClass('default_layers')) {
                ...
                let id_layer = e.layer.feature.properties.id_layer;
                
                generate_infografis(o);
                // console.log('o', o);
                Object.keys(o).map((v, i, a) => {
                    let kode_sub_zona = o['Kode Sub-Zona'];
                    ...
                    if (!['id_layer', 'id_opd', 'id_collection', 'stroke', 'stroke_opacity', 'stroke_width', 'stroke_dash', 'fill', 'fill_opacity', 'icon_name', 'page_detail', 'name', 'group', 'total_foto'].includes(v)) {
                        ...
                        if (v == 'nama_layer') {
                            ...
                            if (id_layer == '131') {
                                content += '<button onclick="lihat_detail_krk(\'' + kode_sub_zona + '\');" class="btn btn-success btn-sm btn-rounded ml-1 mr-1"><i class="fa fa-folder"></i> Info KRK</button>';                                
                            }
                            if (id_layer == '131') {
                                content += '<br><br><button onclick="ajukan_krk(\'' + kode_sub_zona + '\');" class="btn btn-primary btn-lg btn-rounded ml-1 mr-1"><i class="fa fa-location-arrow"></i> Ajukan KRK</button>';                                
                            }
...
<script>
    ...
    function lihat_detail_krk(id) {
        $.ajax({
            type: "POST",
            url: "<?= base_url('peta/lihat_krk') ?>",
            data: {
                id : id,
                lat : $('#krk_klik_lat').val(), 
                lng : $('#krk_klik_lng').val(), 
            },
            dataType: "JSON",
            beforeSend: function(res) {
                Swal.fire({
            ...
            success: function(res) {
                // console.log(res.data.kdb.data)
                var html = '';

                if (res.status == true) {

                    var kdb = '';
                    kdb += '<label>Koefisien Dasar Bangunan</label><br>';
                    if (res.data.kdb.data != '-') {
                        $.each( res.data.kdb.data, function( key, value ) {
                            kdb += '<label>'+key+' : '+value+'</label><br>';
                            // alert( key + ": " + value );
                        });
                    } else {
                        kdb += '<label>Tidak ada data</label>';
                    }
                    

                    var klb = '';
                    klb += '<label>Koefisian Lantai Bangunan</label><br>';
                    if (res.data.klb.data != '-') {
                        $.each( res.data.klb.data, function( key, value ) {
                            klb += '<label>'+key+' : '+value+'</label><br>';
                            // alert( key + ": " + value );
                        });
                    } else {
                        klb += '<label>Tidak ada data</label>';
                    } 

                    var kdh = '';
                    kdh += '<label>Koefisien Daerah Hijau</label><br>';
                    if (res.data.kdh.data != '-') {
                        $.each( res.data.kdh.data, function( key, value ) {
                            kdh += '<label>'+key+' : '+value+'</label><br>';
                            // alert( key + ": " + value );
                        });
                    } else {
                        kdh += '<label>Tidak ada data</label>';
                    }

                    var gsb = '';
                    gsb += '<label>Garis Sempadan Pantai</label><br>';
                    if (res.data.gsb.data != '-') {
                        $.each( res.data.gsb.data, function( key, value ) {
                            gsb += '<label>'+key+' : '+value+'</label><br>';
                            // alert( key + ": " + value );
                        });
                    } else {
                        gsb += '<label>Tidak ada data</label>';
                    } 

                    var ktb = '';
                    ktb += '<label>Koefisien Tapak Basement</label><br>';
                    if (res.data.ktb.data != '-') {
                        $.each( res.data.ktb.data, function( key, value ) {
                            ktb += '<label>'+key+' : '+value+'</label><br>';
                            // alert( key + ": " + value );
                        });
                    } else {
                        ktb += '<label>Tidak ada data</label>';
                    }

                    var ktgbgn = '';
                    ktgbgn += '<label>Ketinggian Bangunan</label><br>';
                    if (res.data.ktgbgn != '-') {
                        $.each( res.data.ktgbgn, function( key, value ) {
                            ktgbgn += '<label>'+value+'</label><br>';
                            // alert( key + ": " + value );
                        });
                    } else {
                        ktgbgn += '<label>Tidak ada data</label>';
                    }

                    var bgnizn = '';
                    bgnizn += '<label>Kegiatan Diperbolehkan Berizin</label><br>';
                    if (res.data.bgnizn.data != '-') {
                        $.each( res.data.bgnizn, function( key, value ) {
                            bgnizn += '<label>'+value+'</label><br>';
                            // alert( key + ": " + value );
                        });
                    } else {
                        bgnizn += '<label>Tidak ada data</label>';
                    }

                    var bgntbt = '';
                    bgntbt += '<label>Kegiatan Terbatas</label><br>';
                    if (res.data.bgntbt.data != '-') {
                        $.each( res.data.bgntbt, function( key, value ) {
                            bgntbt += '<label>'+value+'</label><br>';
                            // alert( key + ": " + value );
                        });
                    } else {
                        bgntbt += '<label>Tidak ada data</label>';
                    }

                    var bgnbst = '';
                    bgnbst += '<label>Kegiatan Bersyarat</label><br>';
                    if (res.data.bgnbst.data != '-') {
                        $.each( res.data.bgnbst, function( key, value ) {
                            bgnbst += '<label>'+value+'</label><br>';
                            // alert( key + ": " + value );
                        });
                    } else {
                        bgnbst += '<label>Tidak ada data</label>';
                    }

                    var bgntbs = '';
                    bgntbs += '<label>Kegiatan Tidak Diperbolehkan</label><br>';
                    if (res.data.bgntbs.data != '-') {
                        $.each( res.data.bgntbs, function( key, value ) {
                            bgntbs += '<label>'+value+'</label><br>';
                            // alert( key + ": " + value );
                        });
                    } else {
                        bgntbs += '<label>Tidak ada data</label>';
                    }

                    var ketrgn = '';
                    ketrgn += '<label>Keterangan</label><br>';
                    if (res.data.ketrgn.data != '-') {
                        $.each( res.data.ketrgn.data, function( key, value ) {
                            ketrgn += '<label>'+key+' : '+value+'</label><br>';
                            // alert( key + ": " + value );
                        });
                    } else {
                        ketrgn += '<label>Tidak ada data</label>';
                    }
                    ...
                    html += '<div class="block">'+
                        ...
                        '<ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">'+
                            '<li class="nav-item" role="presentation">'+
                                '<button class="nav-link active" id="pills-kdb-tab" data-toggle="pill" data-target="#pills-kdb" type="button" role="tab" aria-controls="pills-kdb" aria-selected="true">Koefisien Dasar Bangunan</button>'+
                            '</li>'+
                            '<li class="nav-item" role="presentation">'+
                                '<button class="nav-link" id="pills-klb-tab" data-toggle="pill" data-target="#pills-klb" type="button" role="tab" aria-controls="pills-klb" aria-selected="false">Koefisian Lantai Bangunan</button>'+
                            '</li>'+
                            '<li class="nav-item" role="presentation">'+
                                '<button class="nav-link" id="pills-kdh-tab" data-toggle="pill" data-target="#pills-kdh" type="button" role="tab" aria-controls="pills-kdh" aria-selected="false">Koefisien Daerah Hijau</button>'+
                            '</li>'+
                            '<li class="nav-item" role="presentation">'+
                                '<button class="nav-link" id="pills-gsb-tab" data-toggle="pill" data-target="#pills-gsb" type="button" role="tab" aria-controls="pills-gsb" aria-selected="false">Garis Sempadan Pantai</button>'+
                            '</li>'+
                            '<li class="nav-item" role="presentation">'+
                                '<button class="nav-link" id="pills-ktb-tab" data-toggle="pill" data-target="#pills-ktb" type="button" role="tab" aria-controls="pills-ktb" aria-selected="false">Koefisien Tapak Basement</button>'+
                            '</li>'+
                            '<li class="nav-item" role="presentation">'+
                                '<button class="nav-link" id="pills-ktgbgn-tab" data-toggle="pill" data-target="#pills-ktgbgn" type="button" role="tab" aria-controls="pills-ktgbgn" aria-selected="false">Ketinggian Bangunan</button>'+
                            '</li>'+
                            '<li class="nav-item" role="presentation">'+
                                '<button class="nav-link" id="pills-bgnizn-tab" data-toggle="pill" data-target="#pills-bgnizn" type="button" role="tab" aria-controls="pills-bgnizn" aria-selected="false">Bangunan Izin</button>'+
                            '</li>'+
                            '<li class="nav-item" role="presentation">'+
                                '<button class="nav-link" id="pills-bgntbt-tab" data-toggle="pill" data-target="#pills-bgntbt" type="button" role="tab" aria-controls="pills-bgntbt" aria-selected="false">Kegiatan Terbatas</button>'+
                            '</li>'+
                            '<li class="nav-item" role="presentation">'+
                                '<button class="nav-link" id="pills-bgngst-tab" data-toggle="pill" data-target="#pills-bgngst" type="button" role="tab" aria-controls="pills-bgngst" aria-selected="false">Kegiatan Bersyarat</button>'+
                            '</li>'+
                            '<li class="nav-item" role="presentation">'+
                                '<button class="nav-link" id="pills-bgntbs-tab" data-toggle="pill" data-target="#pills-bgntbs" type="button" role="tab" aria-controls="pills-bgntbs" aria-selected="false">Kegiatan Tidak Diperbolehkan</button>'+
                            '</li>'+
                            '<li class="nav-item" role="presentation">'+
                                '<button class="nav-link" id="pills-ketrgn-tab" data-toggle="pill" data-target="#pills-ketrgn" type="button" role="tab" aria-controls="pills-ketrgn" aria-selected="false">Keterangan</button>'+
                            '</li>'+
                        '</ul>'+
                        
                        '<div class="tab-content" id="pills-tabContent">'+
                            '<div class="tab-pane fade show active" id="pills-kdb" role="tabpanel" aria-labelledby="pills-kdb-tab">'+
                                '<div class="container">'+
                                    '<div class="z-depth-1 grey lighten-4" style="border-radius: 12px; padding: 10px;">'+
                                        kdb+
                                    '</div>'+
                                '</div>'+
                            '</div>'+
                            '<div class="tab-pane fade" id="pills-klb" role="tabpanel" aria-labelledby="pills-klb-tab">'+
                                ...
                                '</div>'+
                            '</div>'+
                            '<div class="tab-pane fade" id="pills-kdh" role="tabpanel" aria-labelledby="pills-kdh-tab">'+
                                '<div class="container">'+
                                    '<div class="z-depth-1 grey lighten-4" style="border-radius: 12px; padding: 10px;">'+
                                        kdh+
                                    '</div>'+
                            ...
                            '<div class="tab-pane fade" id="pills-gsb" role="tabpanel" aria-labelledby="pills-gsb-tab">'+
                                ...
                                '</div>'+
                            '</div>'+
                            '<div class="tab-pane fade" id="pills-ktb" role="tabpanel" aria-labelledby="pills-ktb-tab">'+
                                '<div class="container">'+
                                    '<div class="z-depth-1 grey lighten-4" style="border-radius: 12px; padding: 10px;">'+
                                        ktb+
                                    '</div>'+
                                '</div> '+
                            '</div>'+
                            '<div class="tab-pane fade" id="pills-ktgbgn" role="tabpanel" aria-labelledby="pills-ktgbgn-tab">'+
                                '<div class="container">'+
                                    '<div class="z-depth-1 grey lighten-4" style="border-radius: 12px; padding: 10px;">'+
                                        ktgbgn+
                                    '</div>'+
                                '</div> '+
                            '</div>'+
                            '<div class="tab-pane fade" id="pills-bgnizn" role="tabpanel" aria-labelledby="pills-bgnizn-tab">'+
                                '<div class="container">'+
                                    '<div class="z-depth-1 grey lighten-4" style="border-radius: 12px; padding: 10px;">'+ 
                                        bgnizn+
                                    '</div>'+
                                '</div> '+
                            '</div>'+
                            '<div class="tab-pane fade" id="pills-bgntbt" role="tabpanel" aria-labelledby="pills-bgntbt-tab">'+
                                '<div class="container">'+
                                    '<div class="z-depth-1 grey lighten-4" style="border-radius: 12px; padding: 10px;">'+
                                        bgntbt+
                                    '</div>'+
                                '</div> '+
                            '</div>'+
                            '<div class="tab-pane fade" id="pills-bgngst" role="tabpanel" aria-labelledby="pills-bgngst-tab">'+
                                '<div class="container">'+
                                    '<div class="z-depth-1 grey lighten-4" style="border-radius: 12px; padding: 10px;">'+ 
                                        bgnbst+
                                    '</div>'+
                                '</div> '+
                            '</div>'+
                            '<div class="tab-pane fade" id="pills-bgntbs" role="tabpanel" aria-labelledby="pills-bgntbs-tab">'+
                                '<div class="container">'+
                                    '<div class="z-depth-1 grey lighten-4" style="border-radius: 12px; padding: 10px;">'+ 
                                        bgntbs+
                                    '</div>'+
                                '</div> '+
                            '</div>'+
                            '<div class="tab-pane fade" id="pills-ketrgn" role="tabpanel" aria-labelledby="pills-ketrgn-tab">'+
                                '<div class="container">'+
                                    '<div class="z-depth-1 grey lighten-4" style="border-radius: 12px; padding: 10px;">'+ 
                                        ketrgn+
                                    '</div>'+
                    ...
                    $('#modal_krk .konten').html(html);
                    $('#modal_krk').modal('show');
                }
            }
        });
    }

    // function ajukan_krk(id) {

    //     modal_ajukan_krk
    //     Swal.fire({
    //         title: 'Apakah Anda Yakin',
    //         text: "Anda akan mengajukan KRK pada data ini",
    //         type: 'warning',
    ...
    //         if (result.value) {
    //             $.ajax({
    //                 url: "<?= base_url('peta/aksi_ajukan_krk') ?>",
    //                 type: "POST",
    //                 data: {
    //                     id : id,
    //                     lat : $('#krk_klik_lat').val(), 
    ...
    //                     window.open('<?= base_url() . 'peta/ajukan_krk/' ?>'+data.data+'/'+data.lat+'/'+data.lng, '_blank');
    //                 },
    //             });
    ...
    function ajukan_krk(id) {
        $.ajax({
            type: "POST",
            url: "<?= base_url('peta/modal_ajukan_krk') ?>",
            data: {
                id : id,
                lat : $('#krk_klik_lat').val(), 
                lng : $('#krk_klik_lng').val(), 
            ...
            success: function(res) {
                var html = '';
                var base_url_krk = '<?php echo base_url('peta/page_ajukan_krk') ?>';

                if (res.status == true) {
                    console.log(res);
                    var option = '';
                    option += '<option>Silakan pilih kegiatan</option>'; 
                    $.each( res.data, function( key, value ) {
                        option += '<option value="'+value+'">'+value+'</option>';
                    });

                    html += 
                    '<form action="'+base_url_krk+'" method="POST" target="_blank">'+
                        '<input type="hidden" name="lat" id="ajukan_lat" value="'+res.lat+'">'+
                        '<input type="hidden" name="lng" id="ajukan_lng" value="'+res.lng+'">'+
                        '<div class="row">'+
                            '<div class="col-md-3">'+
                                '<label>Kode Sub Zona</label>'+
                            '</div>'+
                            '<div class="col-md-9">'+
                                '<input class="form-control" name="keyword" id="ajukan_keyword"  value="'+res.keyword+'">'+
                            '</div>'+
                        '</div>'+
                        '<div class="row mt-3">'+
                            '<div class="col-md-3">'+
                            ...
                            '</div>'+
                            '<div class="col-md-9">'+
                                '<select class="form-control" name="kegiatan" id="ajukan_kegiatan" style="width: 100%;">'+
                                option+
                                '</select>'+
                            '</div>'+
                        '</div>'+
                    ...
                    $('#modal_ajukan_krk .konten').html(html);
                    $('#modal_ajukan_krk').modal('show');
                }
            }
        });
    }

    $('#form_ajukan').submit(function(e) {
        e.preventDefault(); 

        // let data = new FormData(this);
        // var url = "<?= base_url('peta/aksi_ajukan_krk') ?>"; 
        ...
        //     success: function(data) {
        //         if (data.status == true) {
        //             window.location.href = '<?= base_url('peta/page_ajukan_krk/') ?>'+data.keyword+'/'+data.lat+'/'+data.lng
        //         } else {
